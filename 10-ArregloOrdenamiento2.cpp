...
int main(){
	int Arreglo[10];//en este arreglo se guardaran los valores del archivo txt
	...
	int aux;		//guardara un valor del arreglo para se comparado con otro
	...
	while (getline(Leer, Archivo)){	//utiliza el bucle while y la funcion getline para leer el archivo linea por linea
		...
		int NumEnt =atoi(Archivo.c_str()); //Archivo.c_str() convierte la cadena a caracteres.		atoi() convierte los caracteres a enteros		
		Arreglo[Estado]=NumEnt;//guarda cada valor de la variable "NumEnt" en el Arreglo
